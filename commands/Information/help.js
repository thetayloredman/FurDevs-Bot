...
exports.run = async (client, message, args) => {
    ...
    async function commandHelp(command) {
        ...
        var embed = new Discord.MessageEmbed()
            ...
            .addField('Usage', `\`${prefix}${cmd.help.name} ${cmd.help.usage ? cmd.help.usage : ''}\``)
            .addField('Description', cmd.help.description || 'No Description provided');

        if (cmd.help.aliases) {
            embed.addField('Aliases', `\`${prefix}${cmd.help.aliases.join('`, `>')}\``);
        }
