...
exports.run = async (client, message, args) => {
    ...
    let creditsEmbed = new MessageEmbed()
        ...
        .setColor(`#8800FF`)
        .setTimestamp()
        .setFooter(`Bot Creator: ${creator.tag} | User ID: ${message.author.id}`, creator.displayAvatarURL({ dynamic: true }));
    credits.map((credit) => creditsEmbed.addField(`${credit.names.map((name) => `**${name}**`).join(', ')}`, credit.contributions));
