    ...
    if (message.member.hasPermission('MANAGE_ROLES') || settings.staffMembers.includes(message.author.id)) {
        ...
        if (!role) {
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .setDescription(
                    `The Current Banker Role is ${
                        GuildSettings.bankerRole ? '<@&' + GuildSettings.bankerRole + '>' : 'Not Set!'
        ...
        throw new Error('Only Staff members are allowed to assign what roles should be the banker role');
    }
};
exports.help = {
    name: 'setbankerrole',
    description: 'Change the banker role.',
    usage: '[ Role Mention ]',
    aliases: ['bankerrole']
};
