    ...
    if (args[0] === 'show') {
        if (args[1] === 'guild') {
            ...
            const embed = new MessageEmbed()
                ...
                .addField(
                    `Disabled Commands:`,
                    `\`\`\`${GuildSettings.disabledCommands ? GuildSettings.disabledCommands.forEach((cmd) => cmd.name) : 'None Set'}\`\`\``
                )
                .addField(`Banker Role (Coming Soon):`, `${GuildSettings.bankerRole ? GuildSettings.bankerRole : 'None Set'}`)
                .addField(
                    `Verification Role:`,
                    `${GuildSettings.verificationRole ? message.guild.role.resolve(GuildSettings.verificationRole) : 'None Set'}`
                )
