...
const { verificationLogs } = require('./../../settings.json');

exports.run = async (client, message, args) => {
    await message.delete();
    const settings = await message.guild.settings();
    if (message.member.hasPermission('MANAGE_ROLES') || settings.staffMembers.includes(message.author.id)) {
        if (!args[0]) {
        ...
        if (guildSettings.verificationRole) {
            ...
            message.channel.send(
                `${member} is now verified for \`${message.guild.name}\` Please Give them a warm welcome in the General Chat!${
                    message.guild.id === '731520035717251142' ? "Here's a little copy and paste" : ''
                }`
            );
...
**Here are a few things you need to know to get started:**
        ...
        throw new Error('Only Staff Members with `MANAGE_ROLES` or is in the Database are able to execute this comamnd');
    }
};
