...
async function roleNameResolver(message, roleName) {
    ...
    if (!message.guild) throw new Error(`Invalid: rolename arguments cannot be used outside of a guild.`);
    ...
    function resolveRole(query, guild) {
        if (query instanceof Discord.Role) return guild.roles.has(query.id) ? query : null;
        if (typeof query === 'string' && roleRegex.test(query)) return guild.roles.resolve(roleRegex.exec(query)[1]);
        return null;
    }
}
