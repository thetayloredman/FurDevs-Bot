...
async function usernameResolver(message, username) {
    ...
    if (!message.guild) return userResolver(username);
...
function resolveUser(query, guild) {
    ...
    if (typeof query === 'string') {
        if (userOrMemberRegex.test(query)) return guild.client.users.fetch(userOrMemberRegex.exec(query)[1]).catch(() => null);
