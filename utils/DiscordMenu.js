module.exports = class DiscordMenu {
    ...
    createMessageCollector(uid) {
        let messageCollector = this.channel.createMessageCollector(
            (m) => m.author.id === uid && this.messages.find((m2) => m2.message.toLowerCase() === m.cleanContent.toLowerCase()),
            { time: this.time }
        );
        this.messageCollector = messageCollector;
        messageCollector.on('collect', (m) => {
            this.endCollection();
            let msgTrigger = this.messages.find((m2) => m2.message.toLowerCase() === m.cleanContent.toLowerCase());
